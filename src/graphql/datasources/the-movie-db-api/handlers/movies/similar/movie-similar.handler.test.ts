...
import { handler } from "./movie-similar.handler";
...
describe("DataSources/TheMovieDBApi/Movie-Similar-Query-Handler", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('When the "language" is defined', () => {
    it('should call "RESTDatasource.get" correctly', async () => {
      ...
      expect(mockGet.mock.calls[0][0]).toEqual(`movie/${ID}/similar`);
  ...
  describe("When receive the data", () => {
    it("should return the data correctly", async () => {
      mockGet.mockReturnValueOnce(fixtures.movieSimilar);
      const tmdbAPI = new TheMovieDBAPI();
      const result = await handler(
        {
          id: ID,
      ...
      expect(result).toEqual(fixtures.movieSimilar.results);
    });
  });
});
